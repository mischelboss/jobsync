...
export const AGENT_TOOL_DESCRIPTIONS = {
  ...
  prepare_interview: "Prepare the user for an interview for the job they are currently viewing in JobSync, drawing on one of their own resumes, and save the preparation to that job. The app researches the company and generates the questions itself, then returns them to you grouped by kind: technical, gaps, CV breaks, behavioural, questions the candidate should ask, and — only when company research succeeded — culture and current-situation questions. Call it whenever the user asks about interview preparation, likely questions, how to prepare, or what they will be asked: each turn tells you which page they are on, so never ask them to open a job or confirm they have one open, and treat a no_job result from an earlier turn as stale rather than repeating it. It takes longer than the other tools because it researches the company first. Optionally takes the resume's title; with no title the app uses the resume linked to the job, or their default resume.",
} as const;
